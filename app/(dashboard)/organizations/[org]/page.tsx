...
const Org = ({ params }: any) => {
  ...
  return (
    <main className="max-w-[90%] mx-auto h-fit pb-6">
      ...
      <h1 className="text-4xl md:text-5xl font-semibold mt-3 mb-6 ">Projects.</h1>
      {loading ? (
        ...
        sortedYearEntries.map(([year, projects], index) => (
          <div key={index} className="year-section mb-8">
            <h2 className="text-2xl md:text-3xl font-semibold my-3">{year}</h2>
            <div className="flex gap-5 flex-wrap justify-center md:justify-start"> {/* Modified justify */}
              {projects.map((project: any, projectIndex) => (
                <div
                  key={projectIndex}
                  className="flex flex-col justify-between gap-4 project-card w-full sm:w-64 border px-6 pt-4 pb-3 border-[#dbbb84] rounded-md" // Modified w-full
                >
                  <div>
                    <h1 className="font-bold text-lg md:text-xl">{project.title}</h1>
                    <p className="text-end text-sm">~ {project.student_name}</p>
                    <p className="mt-3 text-sm md:text-base">{project.short_description}</p>
                  </div>
